...
- 2026-09-10（第五批）：**原生 NPU 集成落地（STEP 13/14/15）**，用户选定全 INT8 部署（config 默认改为三个 `*_int8.rknn`，回退只改路径）。新增 `rk3588_dms/service/dms_service.py`（端口 8600）：摄像头独占采集（OpenCV，掉线退避重试+明确日志）→ MJPEG `/video.mjpg`；三模型 INT8 三核并行推理（约 12fps）→ 手写最小 WebSocket `/ws/dms/native` 广播归一化检测（与浏览器 parseDetections 输出同构，零第三方依赖）；`POST /alert` 板端 ffplay 播放四个报警 mp3（`-volume 100`，每 key 冷却 4.5s 与浏览器一致）；`/health`；`--test-image` 静态图帧源调试。`driver-inference.js` 新增 `?infer=native` 模式（缺省仍纯浏览器推理，随时回退）：MJPEG→隐藏 canvas captureStream→原有 #cameraVideo（显示/object-fit/MediaPipe VIDEO/关键区域原样工作）；YOLO 换 WS 缓存，阈值/统一key/phone归属/NMS/drowsy抑制/手动控制/报警时序/模型选择器全沿用浏览器逻辑；音频改 POST key 板端播放；页面与样式零改动（index.html 未动）。已知差异：手机 ROI ±22° 复检 native 下不可用（板端全帧 COCO 承担，极小目标召回略降）；https 页面会拦 http MJPEG/WS（须 http 打开，浏览器有明确提示）。dcztl 烟测通过：WS 握手 101、检测广播（test_0001 帧 soham SafeDriving 板端 0.8481 vs 浏览器 FP ONNX 0.848，一致性闭环）、MJPEG 200、alert 播放与冷却生效。**板上尚无 USB 摄像头（USB 总线仅鼠标）**，真摄像头端到端待插入后验证。修复：service HTTP 立即启动（health 不因无摄像头阻塞）。坑：SSH 远程 pkill/pgrep -f 会自匹配命令行（用 `[s]` 模式规避）。
- 2026-09-10（第四批）：**三模型 INT8 全量量化 + 三核并行全组合实测**。`convert_chaitanya.py` 新增 `--quant-dtype w8a8/w4a16`（W4A16 参数支持，未采用）；`benchmark_parallel.py` 新增 `--variant fp|int8|逗号组合`（按模型切 FP/INT8；修复两处 bug：变体只改存在性检查未改加载路径、单值时越位回退 fp）。dcztl 生成 `soham_best_int8.rknn`（4.2MB）、`yolov8n_coco_int8.rknn`（4.3MB）。**精度（46 张真实座舱图，双方均真 NPU）**：chaitanya 100%类别一致/IoU 0.9825/Δconf 0.0277/零漏检；coco 67/67 一致/IoU 0.99/Δconf 0.0156/5漏6多（阈值边缘抖动）；soham 108/108 一致/IoU 0.9464/Δconf 0.0853/20漏2多——漏检集中在 SafeDriving 安全类置信度下移，涉警类别（Drowsy 0.5→0.383）均低于浏览器 0.6 门槛，判定行为不变。**三核并行实测**：全 FP 74.9ms/13.4FPS → 全 INT8 **43.8ms/22.8FPS**（单核 18.8/22.0/20.4ms，2.29x）→ 混合 int8,fp,int8 68.1ms/14.7FPS。三份 FP-vs-INT8 报告入仓（chaitanya/soham/coco_int8_vs_fp.md）。部署建议全 INT8 首选，比赛 config 默认仍保留 FP，切换只改模型 `rknn` 路径。另：后台任务经 SSH 启动需 `& sleep 3` 保活（此前整链后台化被通道关闭杀死，soham 首次转换静默未执行）；pgrep -f 自匹配会造成假"运行中"。待办不变：STEP 7 摄像头、soham/coco FP 一致性报告、服务化阶段。
- 2026-09-10（第三批）：**INT8 量化完成并真机验证**。用户提供比赛视频（1280x720@60fps，115s，116MB 经 SFTP 上传 dcztl），ffmpeg 均匀抽帧 320 张：46 张测试集入库 `testdata/dms/`（补齐此前缺失的真实测试集），274 张校准集 letterbox 640x640 灰底128（与推理预处理一致，toolkit 内部 resize 变恒等映射）留在真机本地，`dataset.txt` 274 行（gitignore）。`convert_chaitanya.py --int8` 用校准集产出 `chaitanya_best_int8.rknn`（4.2MB）。`compare_outputs.py` 新增 `--rknn2` 模式（FP RKNN 与 INT8 RKNN 双真机对比，报告名 `*_int8_vs_fp.md`）。**真机实测（46 张真实座舱图）**：类别一致率 10/10（100%）、bbox 平均 IoU 0.9825、平均 |Δconf| 0.0277、零漏检零多检；唯一差异样本 test_0148 Cigarette 0.571(FP) vs 0.667(INT8)，INT8 更自信且均超 0.25 阈值，判定行为不变。**INT8 benchmark**：平均 20.372ms / P50 20.1 / P95 23.3 / 理论 49.1 FPS（较 FP 41.7ms 提速 2.05x）。比赛默认配置仍为 FP（保守），切换只需改 config 模型 `rknn` 路径。README INT8 章节更新为实测数据。待办：soham/coco 的 INT8（同流程）、STEP 7 摄像头、soham/coco 的 FP 一致性对比报告。
- 2026-09-10（第二批）：**NPU 三核并行落地并实测**（多核调度优化预留项）。`config/dms.json` 每模型新增 `npu_core` 字段（chaitanya=0/soham=1/coco=2，全局 `runtime.npu_core` 为默认，detector 按 显式参数>模型配置>auto 解析）；`RKNNModel` core 常量 fallback 修正（NPU_CORE_2=4）；新增 `runtime/parallel.py`（`ParallelDetectorGroup` 线程级并行执行器，RKNN 在 C 层释放 GIL 无需多进程，可共享摄像头帧，供原生 DMS 服务复用）与 `tools/benchmark_parallel.py`（顺序 vs 并行对比 + 加速比 + 检出 sanity）。`test_image.py`/`benchmark_rknn.py` 的 `--core` 缺省改为读 config。dcztl 真机上转换出 `soham_best_fp.rknn`（6.6MB）与 `yolov8n_coco_fp.rknn`（7.2MB），三 FP 模型齐全。**真机实测**（face-test.png，warmup10/runs100）：顺序 175.4ms（48.1+51.6+51.1）→ 三核并行 74.9ms，**加速比 2.34x**，理论 13.4 FPS，检出 sanity 正确（soham SafeDriving 0.64 / coco vase 0.48 / chaitanya 阈值内 0 检出）。并行墙钟略高于最慢单模型因三模型共享 DRAM 带宽与 CPU 预处理竞争；若需更快再上 INT8（预计并行 ~40ms）。README 新增"三、NPU 三并行"章节（顺带修正章节编号）。另修 `test_image.py` 动态 Detector 类体内自引用 NameError（改 type() 构建）。STEP 7（板上摄像头）与 soham/coco 的一致性对比（compare_outputs）待做。
- 2026-09-10：**第一阶段停止线达成（STEP 5/6 真机通过）**。真机为 dcztl（192.168.10.182，RK3588 定制板"ztl A588"，根 compatible `rockchip,rk3588`，NPU 节点 `rk3588-rknpu`，RKNPU 驱动 v0.9.8）。过程与修复（均经 GitHub 推送、真机经代理 192.168.10.4:7890 拉取）：① Python 3.12 venv 缺 setuptools 致 toolkit2 报 `pkg_resources` 缺失（`convert_chaitanya.py` 已加明确提示，README 补 `setuptools<81`/`numpy<2` 说明）；② toolkit2 2.x 无 `deinit()`（是 `release()`），finally 崩溃曾把成功转换变成报错（改为兼容探测）；③ toolkit-lite2 2.3+ 推理需显式 4 维 (1,H,W,3) 输入（`runtime/model.py` 已修）；④ toolkit2 2.3+ 禁止 load_rknn 模型进模拟器——`convert_chaitanya.py` 新增 `--validate-image` 在 build 后同对象上做单图验证（低阈值 0.05，类别/IoU≥0.9/Δconf≤0.02 判 PASS）；⑤ `tools/check_rknn_env.py` 与探针兼容非 root 读 debugfs（PermissionError）。板上安装：pip rknn-toolkit-lite2 2.3.2（venv 内）+ librknnrt.so 2.3.2（aarch64，经代理下载后 sudo 装入 /usr/lib）。**实测**：`chaitanya_best_fp.rknn`（7.0MB，FP16 权重）真 NPU 单图推理 46.8ms，Cigarette 检出 0.1190 与浏览器 ONNX 0.1153 对照 bbox 像素级一致 [329,294,486,351]；benchmark（warmup20/runs200，CORE_AUTO）平均 41.662ms / P50 41.2 / P95 46.5 / 理论 24 FPS。STEP 7（板上摄像头）与 soham/coco 转换待做；服务化/WebSocket/双模式仍属后续阶段未开工。
